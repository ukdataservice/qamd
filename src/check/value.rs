...
fn regex_patterns(value: &Value,
                  config: &Config,
                  report: &mut Report) {
    if let Some(ref setting) = config.value_config.regex_patterns {
        include_check!(report.summary.value_regex_patterns,
                       &setting.desc);
...
mod tests {
    ...
    fn test_value_defined_missing_no_label() {
        ...
        if let Some(ref status) = report
                .summary
                .value_defined_missing_no_label {
            assert_eq!(status.fail, 1);
            assert_eq!(status.pass, 0);
        } else {
            assert!(false, concat!("report.summary.",
                                   "value_defined_missing_no_label is",
                                   " None and should be Some(Setting)"))
        }
    ...
    fn test_regex_patterns() {
        ...
        if let Some(ref status) = report.summary.value_regex_patterns {
            assert_eq!(status.fail, 1);
            assert_eq!(status.pass, 0);
        } else {
            assert!(false, concat!("report.summary.value_regex_patterns is",
                                   " None and should be Some(Setting)"))
        }
